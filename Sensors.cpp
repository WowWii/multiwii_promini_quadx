...
void  Baro_init() {
  ...
  delay(5);
  i2c_BMP085_UT_Start(); 
  bmp085_ctx.deadline = currentTime_us+5000;
}
...
uint8_t Baro_update() {                   // first UT conversion is started in init procedure
  if (currentTime_us < bmp085_ctx.deadline) return 0; 
  bmp085_ctx.deadline = currentTime_us+6000; // 1.5ms margin according to the spec (4.5ms T convetion time)
...
void  Baro_init() {
  ...
  ms561101ba_ctx.deadline = currentTime_us+10000; 
}
...
//return 0: no data available, no computation ;  1: new value available  ; 2: no new value, but computation time
uint8_t Baro_update() {                            // first UT conversion is started in init procedure
  if (currentTime_us < ms561101ba_ctx.deadline) return 0; 
  ms561101ba_ctx.deadline = currentTime_us+10000;  // UT and UP conversion take 8.5ms so we do next reading after 10ms 
...
void Sonar_update() {
  if (currentTime_us < srf08_ctx.deadline || (srf08_ctx.state==0 && f.ARMED)) return; 
  srf08_ctx.deadline = currentTime_us;
